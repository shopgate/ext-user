...
module.exports = joi => (
  joi.object().keys({
    prefix: joi.string().trim().regex(defaultAlphaExpr).optional().max(20),
    firstName: joi.string().trim().regex(defaultAlphaExpr).required().min(1).max(100),
    middleName: joi.string().trim().regex(defaultAlphaExpr).optional().max(100),
    lastName: joi.string().trim().regex(defaultAlphaExpr).required().min(1).max(100),
    suffix: joi.string().trim().regex(defaultAlphaExpr).optional().max(100),
    phone: joi.string().trim().regex(phoneExpr).optional().max(20),
    company: joi.string().trim().regex(defaultAlphaExpr).optional().max(100),
    street1: joi.string().trim().regex(defaultSpecCharExpr).regex(streetExpr).required().min(1).max(255),
    street2: joi.string().trim().regex(defaultSpecCharExpr).regex(streetExpr).optional().max(255),
